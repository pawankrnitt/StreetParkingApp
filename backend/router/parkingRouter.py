...
from controller.parkingController import fetch_parking_lots, fetch_parking_lot, add_parking_lot, change_parking_lot_status, remove_parking_lot, fetch_parking_lot_price
...
def get_lots(db: Session = Depends(get_db), current_user: Optional[User] = Depends(get_optional_current_user)):
    # if not logged in, assume CITIZEN role, else use actual role
    role = current_user.role if current_user else "CITIZEN"
    return fetch_parking_lots(db, role)
